...
- 포켓몬 선택은 **미리보기 방식** — 초상화를 클릭하면 화면의 펫이 즉시 바뀌지만 저장되지 않고, Select를 눌러야 확정된다. Cancel·타이틀바 X·Esc로 닫으면 원래 포켓몬으로 복원
  - `previewPokemon()`(화면만) / `changePokemon()`(화면+저장)으로 분리. 저장 경로가 Select 하나뿐이라 닫기 동작이 일관됨
  - 복원은 `windowWillClose`에서 처리 — 버튼뿐 아니라 모든 닫기 경로를 덮는다
  - 확정 시 위치는 유지하고 애니메이션 상태만 Idle로 초기화(`resetToIdle`). 드래그로 옮긴 위치는 사용자의 배치 의도이므로 되돌리지 않음
