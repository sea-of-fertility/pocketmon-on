...
- 목표점 도달 제한 시간: 거리/속도 기반 동적 계산 (`min((거리 / 속도) × 여유배수 + 5초, 상한)`)
  - 여유배수: 활동 빈도의 걷는 시간 비율에서 산출 (빈도 1: 약 4.6배 ~ 빈도 5: 약 1.4배). Walk 중간에 Idle로 쉬는 시간이 있어 실제 이동 비율이 28~92%로 크게 차이남
  - 상한: 수면 타임아웃 × 0.5. 상한이 없으면 조용한 설정에서 제한 시간(최대 296초)이 수면 타임아웃(기본 180초)보다 길어져 목표점 만료 전에 잠들어 기능이 무의미해짐 — 잠들기 전 최소 2회 재시도 보장
