...
- 목표점 도달 제한 시간: 거리/속도 기반 동적 계산 (`(거리 / 속도) × 여유배수 + 5초`). 여유배수는 활동 빈도의 걷는 시간 비율에서 산출(빈도 1: 약 4.6배 ~ 빈도 5: 약 1.4배) — 고정 상수는 화면 크기/속도 설정에 따라 적정값이 크게 달라져 부적합. 설정 UI 미노출(교착 방지용 내부 안전장치)
